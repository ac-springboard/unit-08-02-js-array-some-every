...
function hasAZero(num) {
  return Array.from(num.toString()).some(d => d === '0');
}
...
function hasNoDuplicates(arr) {
  // console.log( arr.map( function(val) { return val; } ) );
  // const arrClone=arr.map( val => val );
  // return arrClone.some( val => (arrClone.shift()).includes(val));
  // const fsome = (val, idx ) => {
  //   clonarr = arr.map( )
  // };
  return arr.map(val => val).some((val, idx, narr) => {
    narr.shift();
    return !(narr.includes(val));
  });
}
